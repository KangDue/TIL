...
p2. NxN 정사각형은 테두리부분은 특수 약품처리(미생물 x)
3. 최초 각 군집의 위치y 위치x, 군집내 미생물 수, 이동 방향 주어짐
4. 한 스텝마다 이동
5. 약품에 닿으면 절반이 죽고 이동방향이 반대로 바뀜.
6. 살아남은 미생물 수는 원래 수 / p2 에소 소수점 버린값 = 2로 나눈 몫
7. 1마리가 벽에닿으면 사라짐
...
#                 mv = max(values,key=lambda x:aos[x][p2])#가장 큰 군집 번호
#                 easum = 0
#                 for each in values:
#                     easum += aos[each][p2]
#                     aos[each][p2] = 0 #0으로 만들면 알아서 이제 걸러짐.
#                 aos[mv][p2] = easum #가장 큰놈 따라감.
#         M-=1
#     #print(*aos,sep='\n')
#     print(f'#{t+1} {sum(map(lambda x:x[p2],aos))}')
