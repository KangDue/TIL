...
p2. 하나의 그래프에는 많은 신장트리가 존재할 수 있다.
    (모든점을 연결하는 여러 트리가 존재)
...
p2. n개의 정점을 가지는 그래프에 대해 반드시 (n-1)개의 간선만을 사용해야 한다.
3. 사이클이 포함되어서는 안된다.
...
- MST가 1)최소 비용의 간선으로 구성됨 ,p2) 사이클을 포함하지 않음
...
- p2. 정렬된 간선 리스트에서 순서대로 사이클을 형성하지 않는 간선을 선택한다.(=이미 포함되어 있지 않은)
    - 즉, 가장 낮은 가중치를 먼저 선택
    - 사이클을 형성하는 간선을 제외
...
- p2. 앞 단계에서 만들어진 MST 집합에 인접한 정점들 중에서 최소 간선으로 연결된 정점을 선택하여 트리를 확장한다.
